...
def get_contour_area(contours):
    all_areas = {}
    for cnt in contours:
        area= cv2.contourAarea(cnt)
        all_areas.append(area)
    return all_areas

sorted_contours= sorted(,key=cv2.contourArea,reverse = True)
...
if __name__ == "__main__":
    ...
    outputStream = cs.putVideo("Front Camera", w,h)
    ...
    while True:
        ...
        cv2.circle(image, ((w)/2,(h)/2),5,(255,0,0),cv2.FILLED) 
        ...
        for cnt in contours:
            ...
            cv2.circle(image, ((x+w/2),(y+h/2),10,(255,0,0),cv2.FILLED))
            x_center=x+w/2
            y_center=y+h/2
        ...
        # Find center of contour and puts circles around them